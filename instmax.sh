...
USER_CONFIG_PATH="${HOME}/e3max_data/config"
MOONRAKER_CONFIG="${HOME}/e3max_data/config/moonraker.conf"
...
function preflight_checks {
    ...
    if [ "$(sudo systemctl list-units --full -all -t service --no-legend | grep -F 'klipper-e3max.service')" ]; then
